...
def copytree(src:Path, dst:Path, exts=None):
    ...
    return n
def write_index_ts(target_dir:Path, pattern=r".*\.(ts|tsx)$", header="// auto-generated by builder\n"):
    files=[]
    for root,_,fs in os.walk(target_dir):
        for f in fs:
            if re.match(pattern, f) and not f.startswith("index."):
                pth = Path(root)/f; rel = pth.relative_to(target_dir)
                mod = str(rel.with_suffix("")).replace("\\","/"); files.append(mod)
...
def main():
    ap = argparse.ArgumentParser(description="oriel builder")
    ...
    D_SCHEMAS = ensure(proj/'src/types/schemas_ext')
    D_TYPES   = ensure(proj/'src/shards/types')
    D_SHARDS  = ensure(proj/'src/shards/data')
    D_MODULES = ensure(proj/'src/modules')
    D_MODES   = ensure(proj/'src/modes')
    D_COMP    = ensure(proj/'src/components/extra')
    D_SHADERS = ensure(proj/'src/shaders')
    D_ASSETS  = ensure(proj/'src/assets/extra')
    with tempfile.TemporaryDirectory() as td:
        ...
        for key, paths in candidates.items():
            for rel in paths:
                pth = root/rel
                if pth.exists(): found[key]=pth; break
        stats={}
        ...
        p('done.'); [p(f"- copied {k}: {v} files") for k,v in stats.items()]; p('indices:', idx1, idx2)
